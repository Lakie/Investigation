...
import com.unesasoft.exception.InvalidOldPasswordException;
import com.unesasoft.exception.UserAlreadyExistException;
import com.unesasoft.exception.UserNotFoundException;
...
import com.unesasoft.util.GenericResponse;
...
/**
 * Created by Slavyanin on 24.07.2015.
 */
@Controller
...
//        return new GenericResponse("success");
    ...
    public String confirmRegistration(final Locale locale, final Model model, @RequestParam("token") final String token) {
        final VerificationTokenDTO verificationToken = userService.getVerificationToken(token);
    ...
    public GenericResponse resendRegistrationToken(final HttpServletRequest request, @RequestParam("token") final String existingToken) {
        final VerificationTokenDTO newToken = userService.generateNewVerificationToken(existingToken);
        final UserDTO user = userService.getUser(newToken.getToken());
        final String appUrl = "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath();
    ...
    public String showChangePasswordPage(final Locale locale, final Model model, @RequestParam("id") final long id, @RequestParam("token") final String token) {
        final PasswordResetTokenDTO passToken = userService.getPasswordResetToken(token);
        final UserDTO user = passToken.getUser();
    ...
    private final SimpleMailMessage constructResendVerificationTokenEmail(final String contextPath, final Locale locale, final VerificationTokenDTO newToken, final UserDTO user) {
    ...
    private final SimpleMailMessage constructResetTokenEmail(final String contextPath, final Locale locale, final String token, final UserDTO user) {
    ...
    private UserDTO createUserAccount(final UserBean accountDto) {
